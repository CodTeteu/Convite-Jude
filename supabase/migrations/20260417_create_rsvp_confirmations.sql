...
create index if not exists idx_rsvp_confirmations_created_at
  on public.rsvp_confirmations (created_at desc);

create index if not exists idx_rsvp_confirmations_event_slug
  on public.rsvp_confirmations (event_slug);

create index if not exists idx_rsvp_confirmations_status
  on public.rsvp_confirmations (attendance_status);

create index if not exists idx_rsvp_confirmations_guest_name
  on public.rsvp_confirmations using gin (to_tsvector('simple', guest_name));

create index if not exists idx_rsvp_confirmations_phone
  on public.rsvp_confirmations (phone);
